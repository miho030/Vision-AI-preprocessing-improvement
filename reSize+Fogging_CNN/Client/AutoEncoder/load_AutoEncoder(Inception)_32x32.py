...
# 차원축소 데이터 저장 경로 지정
npyResDir = "./auto_npyRes/fog_auto_I_32/"
...
# Encoder를 사용하여 차원 축소
if choice == "1":
    encoder = load_model(npyResDir + "fog_autoencoder_inception_32x32_dogcat.h5")
    train_features_reduced = encoder.predict(train_features)
    test_features_reduced = encoder.predict(test_features)
elif choice == "2":
    encoder = load_model(npyResDir + "fog_autoencoder_inception_32x32_FEB2013.h5")
    train_features_reduced = encoder.predict(train_features)
    test_features_reduced = encoder.predict(test_features)
...
print("\n\n" + "=" * 60)
print("--- Fogging + (load)AutoEncoder (InceptionV3) | Size(32x32) TimeSet ---")
