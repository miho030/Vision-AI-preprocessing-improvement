...
# 차원축소 데이터 저장 경로 지정
npyResDir = "./auto_npyRes/fog_auto_M_22/"
...
# Encoder를 사용하여 차원 축소
if choice == "1":
    encoder = load_model(npyResDir + "fog_autoencoder_mobilenet_22x22_dogcat.h5")
    train_features_reduced = encoder.predict(train_features)
    test_features_reduced = encoder.predict(test_features)
elif choice == "2":
    encoder = load_model(npyResDir + "fog_autoencoder_mobilenet_22x22_FEB2013.h5")
    train_features_reduced = encoder.predict(train_features)
    test_features_reduced = encoder.predict(test_features)
print("Encoder model Loaded.")

# npy 파일 저장 경로 수정
choice_dir = os.path.join(npyResDir, f"{choice}/")
...
def measure_npy_file_sizes(directory):
    ...
    np.save(choice_dir + "train_features_pca.npy", train_features_reduced)
    np.save(choice_dir + "test_features_pca.npy", test_features_reduced)
    np.save(choice_dir + "train_labels.npy", training_set.classes)
    np.save(choice_dir + "test_labels.npy", test_set.classes)
    print(f" -> PCA data saved at {choice_dir}\n" + "="*60)
...
measure_npy_file_sizes(choice_dir)
...
print("\n\n" + "=" * 60)
print("--- Fogging + (load)AutoEncoder (MobileNet) | Size(22x22) TimeSet ---")
